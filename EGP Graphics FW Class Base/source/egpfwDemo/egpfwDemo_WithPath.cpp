...
//Modified by David Horntvedt and Brendon Cote to implement the RenderPath
...
void setupShaders()
{
	...
	RenderPass* currRenderpass = new RenderPass();
...
void setupSceneBloom()
{
	...
	moonPass.setCurrentProgram(testTextureProgramIndex);
	moonPass.setPipelineStage(sceneFBO);
	moonPass.setVAO(vao + sphere8x6Model);
	moonPass.addTexture(TextureData(GL_TEXTURE_2D, GL_TEXTURE0, tex[moonTexHandle_dm]));
	moonPass.addUniform(UniformFloatMatrix(glslCommonUniforms[testTextureProgramIndex][unif_mvp], 1, 0, &moonModelViewProjectionMatrix));

	earthPass.setCurrentProgram(phongProgramIndex);
	earthPass.setPipelineStage(sceneFBO);
	earthPass.setVAO(vao + sphereHiResObjModel);
	earthPass.addTexture(TextureData(GL_TEXTURE_2D, GL_TEXTURE1, tex[earthTexHandle_sm]));
	earthPass.addTexture(TextureData(GL_TEXTURE_2D, GL_TEXTURE0, tex[earthTexHandle_dm]));
	earthPass.addUniform(UniformFloatMatrix(glslCommonUniforms[phongProgramIndex][unif_mvp], 1, 0, &earthModelViewProjectionMatrix));

	//Add them to the global render path.
	globalRenderPath.addPass(moonPass);
	globalRenderPath.addPass(earthPass);
}
...
void setupBloom()
{
	...
	//Bright pass
	brightPass.setCurrentProgram(bloomBrightProgramIndex);
	...
	hblur1.setCurrentProgram(bloomBlurProgramIndex);
	brightPass.setVAO(vao + fsqModel);
	hblur1.setPipelineStage(hblurFBO_d2);
	hblur1.addColorTarget(FBOTargetColorTexture(brightFBO_d2, 0, 0));
	hblur1.addUniform(UniformFloat(currentUniformSet[unif_pixelSizeInv], UNIF_VEC2, 1, pixSzInv.set(pixelSizeInv[brightFBO_d2].x, 0.0f).v));

	//First vertical blur
	vblur1.setCurrentProgram(bloomBlurProgramIndex);
	brightPass.setVAO(vao + fsqModel);
	vblur1.setPipelineStage(vblurFBO_d2);
	vblur1.addColorTarget(FBOTargetColorTexture(hblurFBO_d2, 0, 0));
	vblur1.addUniform(UniformFloat(currentUniformSet[unif_pixelSizeInv], UNIF_VEC2, 1, pixSzInv.set(0.0f, pixelSizeInv[hblurFBO_d2].y).v));

	//Second horizontal blur
	hblur2.setCurrentProgram(bloomBlurProgramIndex);
	brightPass.setVAO(vao + fsqModel);
	hblur2.setPipelineStage(hblurFBO_d4);
	hblur2.addColorTarget(FBOTargetColorTexture(vblurFBO_d2, 0, 0));
	hblur2.addUniform(UniformFloat(currentUniformSet[unif_pixelSizeInv], UNIF_VEC2, 1, pixSzInv.set(pixelSizeInv[vblurFBO_d2].x, 0.0f).v));

	//Second vertical blur
	vblur2.setCurrentProgram(bloomBlurProgramIndex);
	brightPass.setVAO(vao + fsqModel);
	vblur2.setPipelineStage(vblurFBO_d4);
	vblur2.addColorTarget(FBOTargetColorTexture(hblurFBO_d4, 0, 0));
	vblur2.addUniform(UniformFloat(currentUniformSet[unif_pixelSizeInv], UNIF_VEC2, 1, pixSzInv.set(0.0f, pixelSizeInv[hblurFBO_d4].y).v));

	//Third horizontal blur
	hblur3.setCurrentProgram(bloomBlurProgramIndex);
	brightPass.setVAO(vao + fsqModel);
	hblur3.setPipelineStage(hblurFBO_d8);
	hblur3.addColorTarget(FBOTargetColorTexture(vblurFBO_d4, 0, 0));
	hblur3.addUniform(UniformFloat(currentUniformSet[unif_pixelSizeInv], UNIF_VEC2, 1, pixSzInv.set(pixelSizeInv[vblurFBO_d4].x, 0.0f).v));

	//Third vertical blur
	vblur3.setCurrentProgram(bloomBlurProgramIndex);
	brightPass.setVAO(vao + fsqModel);
	vblur3.setPipelineStage(vblurFBO_d8);
	vblur3.addColorTarget(FBOTargetColorTexture(hblurFBO_d8, 0, 0));
	vblur3.addUniform(UniformFloat(currentUniformSet[unif_pixelSizeInv], UNIF_VEC2, 1, pixSzInv.set(0.0f, pixelSizeInv[hblurFBO_d8].y).v));

	//Finally, composite them all together with the last pass.
	composite.setCurrentProgram(bloomBlendProgramIndex);
	brightPass.setVAO(vao + fsqModel);
	...
	globalRenderPath.addPass(brightPass);
	globalRenderPath.addPass(hblur1);
	globalRenderPath.addPass(vblur1);
	globalRenderPath.addPass(hblur2);
	globalRenderPath.addPass(vblur2);
	globalRenderPath.addPass(hblur3);
	globalRenderPath.addPass(vblur3);

	globalRenderPath.addPass(composite);
}
...
void renderSkybox()
{
	//switch over to render pass/path
	currentProgramIndex = testTextureProgramIndex;
	currentProgram = glslPrograms + currentProgramIndex;
	currentUniformSet = glslCommonUniforms[currentProgramIndex];
	egpActivateProgram(currentProgram);
	...
	*/
		...
		// FINAL DISPLAY: RENDER FINAL IMAGE ON FSQ TO BACK BUFFER
	{
		...
		if (testDrawAxes)
		{
			...
			currentUniformSet = glslCommonUniforms[currentProgramIndex];
			egpActivateProgram(currentProgram);
			egpSendUniformFloatMatrix(currentUniformSet[unif_mvp], UNIF_MAT4, 1, 0, projectionMatrix[activeCamera].m);
